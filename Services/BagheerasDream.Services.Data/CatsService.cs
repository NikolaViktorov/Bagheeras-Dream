namespace BagheerasDream.Services.Data
{
	...
	public class CatsService : ICatsService
	{
		...
		public async Task<ICollection<CatViewModel>> GetAll(Gender gender)
		{
			...
			{
				var colors = c.Colour.Split("_", StringSplitOptions.RemoveEmptyEntries).ToArray();
				for (int i = 0; i < colors.Length; i++)
				{
					if (colors[i] != "and")
					{
						var color = colors[i][0].ToString().ToUpper() + colors[i].Substring(1);
						colors[i] = color;
					}
				}
				c.Colour = string.Join(" ", colors);
			}
